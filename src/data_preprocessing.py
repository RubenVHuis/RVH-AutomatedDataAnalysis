...
class DataPreprocessor:
    ...
    def generate_report(self, output_path: str = "preprocessing_report.csv"):
        """
        Generate a comprehensive CSV report of all preprocessing operations.
        ...
        report_rows.append(
            {
                "Section": "Overview",
                "Detail": "Total Steps",
                "Value": len(self.preprocessing_steps),
                "Info": "",
            }
        )
        report_rows.append(
        ...
        for i, step in enumerate(self.preprocessing_steps, 1):
            ...
            if step_type == "encode_categorical":
                report_rows.append(
                    {
                        "Section": f"Step {i}",
                        "Detail": "Method",
                        "Value": step["method"],
                        "Info": f"Drop First: {step['drop_first']}",
                    }
                )
                report_rows.append(
                    {
                        "Section": f"Step {i}",
                        "Detail": "Columns Encoded",
                        "Value": ", ".join(step["columns"]),
                        "Info": f"Count: {step['n_columns_encoded']}",
                    }
                )
                report_rows.append(
                    {
                        "Section": f"Step {i}",
                        "Detail": "Shape Change",
                        "Value": f"{step['original_shape']} → {step['new_shape']}",
                        "Info": "",
                    }
                )
                ...
                report_rows.append(
                    {
                        "Section": f"Step {i}",
                        "Detail": "Columns Scaled",
                        "Value": ", ".join(step["columns"]),
                        "Info": f"Count: {step['n_features_scaled']}",
                    }
                )

            elif step_type == "balance_dataset":
                report_rows.append(
                    {
                        "Section": f"Step {i}",
                        "Detail": "Method",
                        "Value": step["method"],
                        "Info": f"Target: {step['target_column']}",
                    }
                )
                report_rows.append(
                    {
                        "Section": f"Step {i}",
                        "Detail": "Original Distribution",
                        "Value": str(step["original_distribution"]),
                        "Info": "",
                    }
                )
                report_rows.append(
                    {
                        ...
                        "Value": str(step["new_distribution"]),
                        "Info": "",
                    }
                )
                report_rows.append(
                    ...
                    report_rows.append(
                        {
                            ...
                            "Detail": "Columns Kept",
                            "Value": ", ".join(step["columns"]),
                            "Info": f"Count: {step['n_features_kept']}",
                        }
                    )
                    report_rows.append(
                        {
                            "Section": f"Step {i}",
                            "Detail": "Columns Removed",
                            "Value": ", ".join(step["removed_columns"]),
                            "Info": f"Count: {step['n_features_removed']}",
                        }
                    )
                else:  # remove
                    report_rows.append(
                        {
                            "Section": f"Step {i}",
                            "Detail": "Action",
                            "Value": "Remove specified features",
                            "Info": "",
                        }
                    )
                    report_rows.append(
            ...
            elif step_type == "apply_pca":
                report_rows.append(
                    {
                        "Section": f"Step {i}",
                        "Detail": "Components",
                        "Value": step["n_components"],
                        "Info": f"Variance Threshold: {step['variance_threshold']}",
                    }
                )
                report_rows.append(
                    {
                        ...
                        "Value": ", ".join(step["original_features"]),
                        "Info": f"Count: {step['n_original_features']}",
                    }
                )
                report_rows.append(
                    {
                        "Section": f"Step {i}",
                        "Detail": "Total Variance Explained",
                        "Value": f"{step['total_variance_explained']:.2%}",
                        "Info": "",
                    }
                )
                    ...
                    report_rows.append(
                        {
                            ...
                            "Detail": f"PC{j}",
                            "Value": f"{var:.2%}",
                            "Info": f"Cumulative: {cumvar:.2%}",
                        }
                    )
                if step.get("missing_filled"):
                    report_rows.append(
                        {
                            "Section": f"Step {i}",
                            "Detail": "Note",
                            "Value": "Missing values filled with column means",
                            "Info": "",
                        }
                    )

            elif step_type == "apply_famd":
                report_rows.append(
                    {
                        "Section": f"Step {i}",
                        "Detail": "Components",
                ...
                if step.get("explained_variance"):
                    report_rows.append(
                        {
                            "Section": f"Step {i}",
                            "Detail": "Explained Variance",
                            "Value": str(step["explained_variance"]),
                            "Info": "",
                        }
                    )
