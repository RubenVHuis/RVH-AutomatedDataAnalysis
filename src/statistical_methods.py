...
class StatisticalMethods:
    ...
    def independent_ttest(group1: pd.Series, group2: pd.Series,
        ...
        # Perform t-test
        statistic, pvalue = stats.ttest_ind(g1, g2,
    ...
    @staticmethod
    def one_sample_ttest(sample: pd.Series,
        ...
        # Perform t-test
        statistic, pvalue = stats.ttest_1samp(s, population_mean,
                                             alternative=alternative)
    ...
    @staticmethod
    def paired_ttest(before: pd.Series, after: pd.Series,
                    alternative: str = 'two-sided',
                    alpha: float = 0.05) -> dict:
        """
        ...
        statistic, pvalue = stats.ttest_rel(df_temp['before'], df_temp['after'],
                                           alternative=alternative)
    ...
    @staticmethod
    def two_way_anova(data: pd.DataFrame,
                     dependent_var: str,
                     factor1: str,
                     factor2: str,
                     alpha: float = 0.05) -> dict:
    ...
    @staticmethod
    def chi_square_test(var1: pd.Series, var2: pd.Series,
                       alpha: float = 0.05) -> dict:
        """
    ...
    @staticmethod
    def fishers_exact_test(var1: pd.Series, var2: pd.Series,
                          alternative: str = 'two-sided',
                          alpha: float = 0.05) -> dict:
        """
    ...
    def variance_inflation_factor(df: pd.DataFrame, numerical_cols: list) -> pd.DataFrame:
        ...
        if len(df_numeric) < 2:
            return pd.DataFrame({
                'Variable': ['Error'],
                'VIF': [np.nan],
                'Multicollinearity_Level': ['Insufficient data']
            })
        ...
        for i, col in enumerate(numerical_cols):
            try:
                ...
                vif_data.append({
                    'Variable': col,
                    'VIF': np.nan,
                    'Multicollinearity_Level': f'Error: {str(e)}'
                })
    ...
    @staticmethod
    def correlation_ratio(categorical_var: pd.Series, numerical_var: pd.Series,
                         alpha: float = 0.05) -> dict:
        """
        ...
        groups = [df_temp[df_temp['categorical'] == cat]['numerical'].values
                  for cat in categories]
    ...
    @staticmethod
    def spearman_correlation(var1: pd.Series, var2: pd.Series,
                           alpha: float = 0.05) -> dict:
        """
    ...
    @staticmethod
    def thompson_sampling(rewards_per_arm: dict,
    ...
    @staticmethod
    def epsilon_greedy(rewards_per_arm: dict,
        ...
        return {
            ...
            'interpretation': f"Exploit '{best_arm}' with {exploit_prob:.1%} probability, explore others with {epsilon:.1%} probability" if best_arm else "Insufficient data"
        }
